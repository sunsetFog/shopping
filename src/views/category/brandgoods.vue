	  ...
	  slider() {
		...
		search(){
      	...
      	gotoSearch(page=1){
				...
				$http.get('goods.goods.get-goods-brand-list', {keywords:this.inputs,'page':page,'brand_id':this.$route.params.id,as_id:this.zoneId}).then((json) => {
					if(json.result == 1) {
						this.loading=false;	
...
<style lang="scss" rel="stylesheet/scss" scoped>
	#search{
		.osll{min-height: 100vh;background: #FFF;margin-top: 3rem;box-sizing: border-box;}
	// 	@descendent wrapper {
    //     overflow: scroll;
    // }
		.page-loadmore-wrapper2{height: 100vh;overflow: hidden;}
		.uou{position: fixed;z-index: 99;	top:0;	transition: .2s;
	...
	.el-input-group__append .el-button.el-button--default{background: #F5F5F5;    padding-top: 0.5625rem;
		line-height: 1rem;
