...
export default {
  ...
  methods: {
    ...
    initPayType(price) {
      ...
      $http.get('plugin.store-cashier.frontend.payment.list', { "store_id": this.$route.params.store_id ,"price":price}, "生成中...").then(function (response) {
    ...
    confirm(btn) {
      ...
      if (type == 1) {
        ...
        that.$router.push(this.fun.getUrl('alipay', { status: 2, recharge_money: 22, order_pay_id: this.order_pay_id, uid: this.uid, pid: this.uid }));
      } else if (type == 9) {
        that.appWeiPay();
      } else if (type == 10) {
        that.appAliPay();
    ...
    balancePay(password) {
      var that = this;
      let json = { order_pay_id: that.order_pay_id, "i": that.fun.getKeyByI(), "type": that.fun.getTyep(), pid: this.uid };

      if (!this.fun.isTextEmpty(password)) {
        json = { order_pay_id: that.order_pay_id, "i": that.fun.getKeyByI(), "type": that.fun.getTyep(), payment_password: password, pid: this.uid };
      }
    ...
    cashPay() {
      var that = this;
      $http.get('plugin.store-cashier.frontend.payment.cashPay.cashPay', { order_pay_id: this.order_pay_id, "i": this.fun.getKeyByI(), "type": this.fun.getTyep(), pid: this.uid }).then(function (response) {
        if (response.result == 1) {
          MessageBox.alert(response.msg, '提示');
          that.$router.push(that.fun.getUrl('member'));
        } else {
    ...
    appWeiPay() {
      var that = this;
      $http.get('order.merge-pay.wechatAppPay', { order_pay_id: that.order_pay_id, "i": that.fun.getKeyByI(), pid: this.uid }).then(function (response) {
        if (response.result == 1) {
    ...
    appAliPay() {
      var that = this;
      $http.get('order.merge-pay.alipayAppRay', { order_pay_id: that.order_pay_id, "i": that.fun.getKeyByI(), pid: this.uid }).then(function (response) {
        if (response.result == 1) {
    ...
    getMultipleOrderInfo(val) {
      ...
      $http.get('order.merge-pay', { order_ids: this.order_ids, "i": this.fun.getKeyByI(), "type": this.fun.getTyep(), pid: this.uid }).then(function (response) {
    ...
    getWeChatPayParams() {
      //order.pay.wechatPay
      var that = this;
      var url = 'order.merge-pay.wechatPay';
      if (window.isMiniprogram) {
        var url = 'order.merge-pay.wechatPay&client_type=2&app_type=wechat';
      }
      $http.get(url, { order_pay_id: this.order_pay_id, "i": this.fun.getKeyByI(), "type": this.fun.getTyep(), pid: this.uid }).then(function (response) {
        if (response.result == 1) {
    ...
    getWeChatPayParams2() {
      //order.pay.wechatPay
      var that = this;
      $http.get('order.merge-pay.cloudWechatPay', { order_pay_id: this.order_pay_id, pid: this.uid }).then(function (response) {
        if (response.result == 1) {
    ...
    btnclass(type) {
      if (type == 1 || type == 6 || type == 9 || type == 22) {
        return 'bg_wechat';
