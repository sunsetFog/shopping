...
export default {
  ...
  methods:
    ...
    initEup(data) {
      let that = this;
      if (data.eup_pay == null || data.eup_pay == undefined || data.eup_pay == '') {
        this.isEup = false;
        return;
      }
      this.isEup = true;
      ...
      initHxbtn(data){
        let that = this;
        if (data.huanxun == null || data.huanxun == undefined || data.huanxun == '') {
          this.isHx = false;
          return;
        }
        this.isHx = true;
      ...
      withdrawToBalance(type) {
        ...
        if (this.checkList.length > 0) {
          ...
          $http.post('withdraw.apply.index', { data: data }, '正在提交数据...').then(function (response) {
            console.log(response.data);
            if (response.result == 1) {
              MessageBox('提示', response.msg);
              that.$router.go(-1);
