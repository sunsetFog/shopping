...
export default {
    ...
    methods: {
        ...
        toPayOrder() {
            ...
            $http.get(payUrl, { pid:this.$route.query.pid?this.$route.query.pid:window.localStorage.getItem("uid"),uid: this.$route.params.uid, order_pay_id: this.$route.params.order_pay_id, "i": this.fun.getKeyByI(), "type": this.fun.getTyep() }, '正在...').then(function (response) {
                if (response.result == 1) {
                    if(that.$route.params.money){
        ...
        toFrozen(){
            var that = this;
            $http.get('plugin.froze.Frontend.Modules.Froze.Controllers.recharge.alipay', { order_sn: this.$route.params.order_pay_id}, '正在充值...').then(function (response) {
