...
<!--setting sun-->
        ...
        <!-- 上传图片 -->
				<section>
					...
					<div>
						<div>
							<el-upload class="upload-demo" :on-change="handleChange" :action="uploadUrl" :data="{'attach':item.name}" :on-preview="handlePreview" :on-remove="handleAptiRemove" :file-list="fileList1" list-type="picture-card" :on-exceed="handleExceed" :before-upload="beforeUpload" :on-success="handleAptiSuccess">

              <div style="width: 100%;height: 100%;background: rgba(45,45,45,0);position: relative;">
              <i class="el-icon-plus"></i>
              <div style="width: 100%;height: 100%;background: rgba(45,45,45,0);position: absolute;left: 0px;top:0px;z-index: 99;" @click.stop="occlusion()" v-if="upNumMax>=item.data.tp_max">
              </div>
              </div>
			...
			<template v-if="item.type == 'diycity'">
				<div>
					<yd-cell-group>
						<yd-cell-item arrow>
							<span slot="left">{{item.data.tp_name}}：</span>
							<input slot="right" style="width:74%;" type="text" @click.stop="showCity = true" @click="openCity(item.name)" v-model="item.value" readonly :placeholder="item.data.tp_name">
						</yd-cell-item>
					</yd-cell-group>
	...
	<mt-datetime-picker ref="picker" type="date" year-format="{value} 年" month-format="{value} 月" date-format="{value} 日" @confirm="setDate" v-model="pickerValue" :startDate="startDate">
	</mt-datetime-picker>
