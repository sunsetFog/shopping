<template>
	<div id="newLife" :class="'newLife'+$store.state.service.lang">
		...
		<div class="titleTip"><div class="spare"></div>免费使用</div>
		<ul class="content">
             <!--手机充值--> 
    	...
    	<div class="titleTip" v-if="plugins.vip"><div class="spare"></div>会员特权专区</div>
    	<ul class="content">
            <!-- 机票 -->
...
.newLifech{
	.m-header{
		...
		.info{
			...
			p{text-align:left;margin-left:5rem;line-height:1.375rem;}
		}
		.change{
			position: absolute;
			right: 1rem;
...
.newLifewei{
	...
	.m-header{
		...
		.info{
			...
			.single{text-align:right;margin-right:5rem;line-height:1.375rem;span{float:right}}
		}
		.change{
			width:20%;
			float:left;
...
.order_all {
	margin-top:0.625rem;
	width:100%; padding:0.875rem 0; color:#666;overflow: hidden;background:#fff}
.order_all a i{ font-size: 28px;}
.order_pub{width: 25%;position: relative;box-sizing: border-box;
float:left; border-left:0.0625rem solid #ebebeb; padding-top:0.125rem; text-align:center; color:#8c8c8c; position:relative;font-size: 12px}
