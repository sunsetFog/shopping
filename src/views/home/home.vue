			...
			<div id="home">
					...
					<div id="brand-box" v-if="!this.fun.isTextEmpty($store.state.temp.default.brand)">
						...
						<ul>
							...
							<li><div class="img"><img src=" "></div><span>乐高</span></li>
							<li><div class="img"><img src=" "></div><span>乐高</span></li> -->
                            <li v-for="item in $store.state.temp.default.brand" @click="gotoBrand(item)"><div class="img"><img :src="item.logo"></div><span>{{item.name}}</span></li>
						</ul>
					</div>
					...
					<div id="limit-time" v-if="!this.fun.isTextEmpty($store.state.temp.default.time_goods)">
						...
						<div class="goods" v-for="item in $store.state.temp.default.time_goods" @click="gotoTimeGood(item.id)">
							<div class="left"><img :src="item.thumb" style="width:100%;height:100%;"/></div>
								...
								<ul>
									<li class="title"><h3>{{item.title}}</h3></li>
									<li class="price">限时价：￥<span>{{item.price}}</span><font>￥{{item.market_price}}</font></li>
									<li class="time">
										<yd-icon class="icon-member-time" custom size="1.625rem" color="#f96587"></yd-icon>
										<!-- <span>还剩2天20:00:59</span> -->
                                        <yd-countdown slot="right" :time="item.has_one_goods_limit_buy.end_time" class="bottom" >
                                            <em style="padding:0.125rem;border-radius:0.125rem;">{%d}</em>天
                                            <em style="padding:0.125rem;border-radius:0.125rem;">{%h}</em><b>:</b>
                                            <em style="padding:0.125rem;border-radius:0.125rem;">{%m}</em><b>:</b>
                                            <em style="padding:0.125rem;border-radius:0.125rem;">{%s}</em>
					                    </yd-countdown>
									</li>
								</ul>
...
<div class="temp" v-if='$store.state.temp.item'>
	...
	<mt-popup v-model="search_show" popup-transition="fade"  position="right" style="width:100%;height:100%;z-index:10000;background-color:#f5f5f5;">
  			<mt-header fixed title="选择区域" style="background:#fff;color:#333;border-bottom:0.0625rem solid #e8e8e8;">
            ...
            <div class="current-city">
                <div v-for="(item,index) in province">
                    <div class="tbs" @click.stop='toggle($event,index)'>
                        ...
                        <div class="li2">
                            <div class="icon"><yd-icon class="iconfont icon-icon_location" custom size="1rem" color="#ff9800"></yd-icon></div>
                    ...
                    <transition name="fade">
                        <div class="tbs3" v-show='sort==index'>
                            <div v-for="(val,i) in item.item" style="line-height:1.875rem;border-bottom:0.0625rem;">
