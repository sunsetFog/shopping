...
export default {
  ...
  methods: {
    ...
    //我的收入
    myIncome(){
      this.$router.push(this.fun.getUrl("MyIncome",{}));
    },
      ...
      $http.get('member.member.isAgent', json).then(function (response) {
        ...
        if (response.result == 1) {
          if (response.data.is_agent == 0)//未开启
          {
            ...
            that.is_agent = true;
            that.getData(); //获取推广数据
            that.getIncomeData();//获取收入数据 --优化新增
          }
        } else {
          console.log();
        }
    ...
    //跳转至收入明细
    gotoDetail(){
      this.$router.push(this.fun.getUrl('incomedetails',{}));
    },
    ...
    getIncomeData(){
      $http.get('finance.earning.earning-count').then(response =>{
          console.log(response)
          this.income_total=response.data.total.grand_total;
          this.canIncome=response.data.total.can_withdraw;
    ...
    getExtensionData(){
      $http.get("finance.income-page.index",{},"...").then(response =>{
        if(response.result==1){
          console.log(response);
          this.userInfo=response.data.info;

          this.extension=response.data.available;

          this.unextension=response.data.unavailable;

          this.shareShow=response.data.parameter.share_page;
        }
      }).catch(error =>{
        console.log(error);
      })
