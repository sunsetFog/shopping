    ...
    methods: {
      ...
      miniWxPay(payParams) {
        ...
        payPost(type) {
          ...
          var url = 'plugin.clock-in.api.clock-in-pay.run-clock-pay';
          if (window.isMiniprogram) {
            var url = 'plugin.clock-in.api.clock-in-pay.run-clock-pay&client_type=2&app_type=wechat';
          }
            $http.get(url, json, "支付中...").then(response => {
                if (response.result == 1) {
                    switch (type) {
                        case 1://微信
                          if (window.isMiniprogram) {
