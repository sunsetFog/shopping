...
export default {
    ...
    methods: {
        ...
        //我的战绩明细
        getMyRecordDetail(){
            $http.get('plugin.clock-in.api.clock-in-member.get-clock-list',{page:this.page}).then(response => {
                console.log(response)
                if(response.result == 1){
        ...
        getMoreDetail(){
            ...
            if (this.page >= this.total_page) {
                ...
                $http.get("plugin.clock-in.api.clock-in-member.get-clock-list",{ page: this.page}, '加载中...').then(response =>{
                    if(response.result == 1){
                        this.loading = false; 
