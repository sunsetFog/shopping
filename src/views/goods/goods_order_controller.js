...
export default {
    ...
    methods: {
        ...
        getDataActionBuy() {
            let that = this;
            let json = { goods_id: _goodsId, total: _total, option_id: _optionsId, "i": this.fun.getKeyByI(), "type": this.fun.getTyep(), member_coupon_ids: this.assembleCoupons(), orders: JSON.stringify(this.assembleDeduction()), address: JSON.stringify(currentAddress), dispatch_type_id: 1, mark: this.$route.params.mark ? this.$route.params.mark : 0 };
        ...
        addAddress() {
            ...
            this.addressName = "";
            // this.strShow=this.fun.isTextEmpty(res.data.street)?false:true;
            this.form.province = "";
            this.form.city = "";
            this.form.district = "";
        ...
        editAddress(id) {
            ...
            this.addressTit = "编辑地址";

            this.isEdit = true;

            $http.get("member.member-address.get-one-address", { address_id: id }, "加载中...").then(res => {
                if (res.result == 1) {
                    console.log(res);
                    this.form.username = res.data.username;
                    this.form.mobile = res.data.mobile;
                    this.form.isDefault = res.data.isdefault == 1 ? true : false;
                    //this.addressName=res.data.province+res.data.city+res.data.district;
                    this.addressName = res.data.province + ' ' + res.data.city + ' ' + res.data.district;
                    // this.strShow=this.fun.isTextEmpty(res.data.street)?false:true;
                    ...
                    if (!this.fun.isTextEmpty(res.data.street)) {
                    ...
                    this.getStreet(res.data.district_id);
                }
        ...
        alterAddresshandle() {
            ...
            if (this.strShow) {
                json = { address_id: this.address_id,username: this.form.username, mobile: this.form.mobile, province: this.form.province, city: this.form.city, district: this.form.district, street: this.form.street, address: this.form.address, isdefault: this.form.isDefault ? 1 : 0 };
            } else {
                json = { address_id: this.address_id,username: this.form.username, mobile: this.form.mobile, province: this.form.province, city: this.form.city, district: this.form.district, address: this.form.address, isdefault: this.form.isDefault ? 1 : 0 };
            }
