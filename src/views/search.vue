	...
	<div id="search" transition="fadeInLeft">
		<div id="soso" class="uou" :class="{'mout':amout}">
			<div class="search">
				...
				<el-input placeholder="请输入内容" v-model="inputs" autofocus @click.native.focus='hidebox'>
			...
			<c-sort :goods='goods' :show='true' v-on:sortIn="sortOut" v-on:showscreen="showbox" text='搜索结果'></c-sort>
		</div>
		...
		<div class='screening' v-if='show'>
			<header>
				<span v-for='val,key in screentitle' @click='changetype(key)' :class="{color:select==key}">{{val}}</span>
			</header>
			<div v-if='show1' id="div1">
				<div v-for='val,key in brand' @click='screenbrand(val.id)'>
					<img :src="val.logo">
					<span>{{val.name}}</span>
				</div>
			</div>
			<div v-if='show2' id="div2">
				<div v-for='val,key in classification1' id="divbox">
					<p @click='classificationtype(val.id,key)'>{{val.name}}<span :class="{color1:classificationkey!=key}"></span></p>
					<div v-for='val1,key1 in val.son' id="divbox1" v-if='classificationkey==key'>
						<p @click='classificationtype1(val1.id,key1)'>{{val1.name}}<span :class="{color1:classificationkey1!=key1}"></span></p>
						<div v-for='val2,key2 in val1.has_many_children' @click='classificationtype2(val2.id,key2)' v-if='classificationkey1==key1' id="divbox2">
							{{val2.name}}
			...
			<div v-if='show3' id="div3">
				<!-- <p v-for='val,key in attr' @click='selectattr(key)'>{{val.name}}<span :class="{color1:!(attrkey.indexOf(key)+1)}"></span></p> -->
				<p v-for='val,key in attr' @click='selectattr(key,$event)'>{{val.name}}<span :class="{color1:val.show}"></span></p>
			</div>
			<div v-if='show4' id="div4">
				<dl v-for='val,key in tagname'>
					<dt>{{val.name}}<span @click='showtagname(key)' v-if="val.value.length > 4">{{showtagnamekey!=key?"更多":"收起"}}<span class="icon" :class="{icon1:showtagnamekey==key}"></span></span></dt>
					<dd :class="{color2:showtagnamekey!=key}" class="div4-dd">
						<span v-for='val1,key1 in val.value' @click='selectname($event,val1.id,key,key1)' :class="{color:val1.record ==2}" :data="val1.id">{{val1.name}}</span>
					</dd>
				</dl>
...
#search {
	...
	.el-input-group__append .el-button.el-button--default{background: #f2f2f2;    padding-top: 0.375rem;
		border-radius: 1.875rem;
		padding-right: 1.8125rem;    border-top-left-radius: 0;
		border-bottom-left-radius: 0;
		}
