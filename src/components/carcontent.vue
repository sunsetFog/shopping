<template>
  <div id="carcontent">
    <div class="branch" v-if="list.goo&&list.set.cat_adv_img"><a :href="list.set.cat_adv_url|href_filters"><img
        :src="list.set.cat_adv_img"/></a></div>
    ...
    <template v-for="item in list.data" v-if='list.set.cat_level==3'>
      ...
      <div class="sote" v-if='list.data'>
        <h3 class="" v-if='item.name'>
          <router-link :to="fun.getUrl('catelist',{id:item.id})">{{item.name}}</router-link>
        </h3>
    ...
    <template v-if="!list.goo">
      <template v-if='list.set'>
        <div class="branch" v-if='list.set.cat_level==2 && $store.state.category.adv_img'><a
